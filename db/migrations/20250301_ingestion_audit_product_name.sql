...
ALTER TABLE assetinfo_sheet_runs
    ADD COLUMN IF NOT EXISTS trust_product_id BIGINT REFERENCES trust_products (id),
    ADD COLUMN IF NOT EXISTS trust_product_name VARCHAR(200);

CREATE INDEX IF NOT EXISTS idx_assetinfo_sheet_runs_product
    ON assetinfo_sheet_runs (trust_product_id);

-- 3. 回填历史 pipeline 记录
UPDATE assetinfo_pipeline_runs r
