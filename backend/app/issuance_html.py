...
def render_upload_page(trust_products: list[dict], username: str) -> str:
    ...
    body = f"""
    <p class="muted"><a href="/">首页</a> / 发行资产明细导入</p>
    <h1>信托产品发行资产明细导入</h1>
    ...
    """
    return _page_shell("发行资产明细导入", body, username)
...
def render_records_page(
    ...
    body = f"""
    <p class="muted"><a href="/">首页</a> / <a href="/issuance/upload">发行导入</a> / 发行资产明细</p>
    <h1>发行资产明细</h1>
    ...
    {pager}
    """
    return _page_shell("发行资产明细", body, username)
